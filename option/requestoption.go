...
// RequestOption is an option for the requests made by the cf-rex API Client
...
// WithServiceKey returns a RequestOption that sets the client setting "service_key".
func WithServiceKey(value string) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.ServiceKey = value
		return r.Apply(WithHeader("X-Auth-User-Service-Key", r.ServiceKey))
	})
}
