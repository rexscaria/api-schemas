...
func TestRadarAs112GetTimeseriesWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.As112.GetTimeseries(context.TODO(), cfrex.RadarAs112GetTimeseriesParams{
		...
		Format:       cfrex.F(cfrex.RadarAs112GetTimeseriesParamsFormatJson),
		Location:     cfrex.F([]string{"string"}),
		Name:         cfrex.F([]string{"main_series"}),
		Protocol:     cfrex.F(cfrex.RadarAs112GetTimeseriesParamsProtocolUdp),
		QueryType:    cfrex.F(cfrex.RadarAs112GetTimeseriesParamsQueryTypeA),
		ResponseCode: cfrex.F(cfrex.RadarAs112GetTimeseriesParamsResponseCodeNoerror),
	})
	if err != nil {
