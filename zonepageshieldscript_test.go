...
func TestZonePageShieldScriptListWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Zones.PageShield.Scripts.List(
		context.TODO(),
		"023e105f4ecef8ad9ca31a8372d0c353",
		cfrex.ZonePageShieldScriptListParams{
			Direction:           cfrex.F(cfrex.ZonePageShieldScriptListParamsDirectionAsc),
			ExcludeCdnCgi:       cfrex.F(true),
			ExcludeDuplicates:   cfrex.F(true),
			ExcludeURLs:         cfrex.F("blog.cloudflare.com,www.example"),
			Export:              cfrex.F(cfrex.ZonePageShieldScriptListParamsExportCsv),
			Hosts:               cfrex.F("blog.cloudflare.com,www.example*,*cloudflare.com"),
			OrderBy:             cfrex.F(cfrex.ZonePageShieldScriptListParamsOrderByFirstSeenAt),
			Page:                cfrex.F("page"),
