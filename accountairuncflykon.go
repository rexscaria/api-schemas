...
// AccountAIRunCfLykonService contains methods and other services that help with
// interacting with the cf-rex API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewAccountAIRunCfLykonService] method instead.
type AccountAIRunCfLykonService struct {
	Options []option.RequestOption
}

// NewAccountAIRunCfLykonService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewAccountAIRunCfLykonService(opts ...option.RequestOption) (r *AccountAIRunCfLykonService) {
	r = &AccountAIRunCfLykonService{}
	r.Options = opts
	return
}
...
func (r AccountAIRunCfLykonExecuteDreamshaper8LcmParams) MarshalJSON() (data []byte, err error) {
	return apijson.MarshalRoot(r)
}

// URLQuery serializes [AccountAIRunCfLykonExecuteDreamshaper8LcmParams]'s query
// parameters as `url.Values`.
func (r AccountAIRunCfLykonExecuteDreamshaper8LcmParams) URLQuery() (v url.Values) {
