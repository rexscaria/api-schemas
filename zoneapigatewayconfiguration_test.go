...
func TestZoneAPIGatewayConfigurationGetWithOptionalParams(t *testing.T) {
...
func TestZoneAPIGatewayConfigurationUpdate(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
		...
		cfrex.ZoneAPIGatewayConfigurationUpdateParams{
			Configuration: cfrex.ConfigurationParam{
				AuthIDCharacteristics: cfrex.F([]cfrex.ConfigurationAuthIDCharacteristicsUnionParam{cfrex.ConfigurationAuthIDCharacteristicsAPIShieldAuthIDCharacteristicParam{
					Name: cfrex.F("authorization"),
					Type: cfrex.F(cfrex.ConfigurationAuthIDCharacteristicsAPIShieldAuthIDCharacteristicTypeHeader),
				}}),
			},
		},
	)
