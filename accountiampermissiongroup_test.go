...
func TestAccountIamPermissionGroupGet(t *testing.T) {
...
func TestAccountIamPermissionGroupListWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Accounts.Iam.PermissionGroups.List(
		...
		cfrex.AccountIamPermissionGroupListParams{
			...
			Name:    cfrex.F("NameOfThePermissionGroup"),
