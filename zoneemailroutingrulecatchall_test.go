...
func TestZoneEmailRoutingRuleCatchAllGet(t *testing.T) {
	...
	_, err := client.Zones.Email.Routing.Rules.CatchAll.Get(context.TODO(), "023e105f4ecef8ad9ca31a8372d0c353")
	if err != nil {
...
func TestZoneEmailRoutingRuleCatchAllUpdateWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Zones.Email.Routing.Rules.CatchAll.Update(
		context.TODO(),
		...
		cfrex.ZoneEmailRoutingRuleCatchAllUpdateParams{
			...
			Matchers: cfrex.F([]cfrex.EmailRuleCatchallMatcherParam{{
				Type: cfrex.F(cfrex.EmailRuleCatchallMatcherTypeAll),
			}}),
