...
func TestZoneCloudConnectorRuleUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Zones.CloudConnector.Rules.Update(
		context.TODO(),
...
func TestZoneCloudConnectorRuleList(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Zones.CloudConnector.Rules.List(context.TODO(), "023e105f4ecef8ad9ca31a8372d0c353")
	if err != nil {
		var apierr *cfrex.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
